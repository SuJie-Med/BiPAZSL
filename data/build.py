...
class ImgDatasetParam(object):
    ...
    DATASETS = {
        "imgroot": '/root/shared-nvme/PSVMA/data',
        "dataroot": '/root/shared-nvme/PSVMA/data/xlsa17/data',
...
def build_dataloader(cfg, is_distributed=False):
    ...
    new_img_files = np.array(new_img_files) #每张图片路径
    label = matcontent['labels'].astype(int).squeeze() - 1  #图片类别标签id

    matcontent = io.loadmat(dataroot + "/" + dataset + "/" + class_embedding + "_splits.mat")
    trainvalloc = matcontent['trainval_loc'].squeeze() - 1  #训练验证集，代表每个图片的id
    test_seen_loc = matcontent['test_seen_loc'].squeeze() - 1  #测试机可见类的id
    test_unseen_loc = matcontent['test_unseen_loc'].squeeze() - 1 #测试集不可见类的id

    att_name = 'att'
    cls_name = matcontent['allclasses_names'] #全部类别名称
    attribute = matcontent[att_name].T #属性

    train_img = new_img_files[trainvalloc]  #每个图片训练数据的路径
    train_label = label[trainvalloc].astype(int)  #每个图片训练的标签
    ...
    # =======================================================
    # ✅ MOD 1: 从 unseen 类随机选取一个类别并注入训练集
    # =======================================================
    unseen_classes = np.unique(label[test_unseen_loc])
    rng = np.random.default_rng(seed=42)  # 固定随机种子，保证复现
    inject_unseen_class = rng.choice(unseen_classes)  # 随机选 unseen 类
    inject_indices = np.where(label == inject_unseen_class)[0]

    print(f"[INFO] Inject unseen class ID = {inject_unseen_class} "
          f"({len(inject_indices)} samples) into training set.")

    # 从 unseen 测试集中排除该类样本
    test_unseen_loc = np.setdiff1d(test_unseen_loc, inject_indices)

     # 构造伪标签与空属性
    inject_imgs = new_img_files[inject_indices]
    inject_labels = np.full(len(inject_indices), -1, dtype=int)
    inject_atts = np.zeros((len(inject_indices), attribute.shape[1]), dtype=np.float32)

    # 拼接到训练集
    train_img = np.concatenate([train_img, inject_imgs])
    train_label = np.concatenate([train_label, inject_labels])
    train_att = np.concatenate([train_att, inject_atts])
    # =======================================================
    ...
    # ================== Debug 输出 ==================
    print(f"[DEBUG] Train set after inject:")
    print(f"  -> train_img: {len(train_img)}")
    print(f"  -> train_label: {len(train_label)} (min={train_label.min()}, max={train_label.max()})")
    print(f"  -> train_att: {train_att.shape}")
    print(f"  -> attribute.shape: {attribute.shape}")
    print(f"  -> test_unseen_loc: {len(test_unseen_loc)} remaining samples")
